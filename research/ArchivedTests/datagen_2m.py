"""
Download 2-minute OHLCV data for all Robinhood-listed cryptos via yfinance.
yfinance caps 2m intervals at 60 days of history.
Output: research/data/2m/<symbol>_usd_2m.csv  (columns: date, close)
"""
...
OUT_DIR = os.path.join(os.path.dirname(__file__), "data", "2m")
...
for ticker in TICKERS:
    ...
    out_path = os.path.join(OUT_DIR, f"{symbol}_usd_2m.csv")

    try:
        df = yf.download(ticker, period="60d", interval="2m", auto_adjust=True, progress=False)
