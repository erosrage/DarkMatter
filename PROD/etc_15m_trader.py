"""
ETC 15m Grid Trader -- Production
...
Optimised params (backtest: train=+43.70%, val=+7.72% over ~60 days on $100k):
...
logging.basicConfig(
    ...
    datefmt="%Y-%m-%d %H:%M:%S",
    handlers=[
        logging.FileHandler(f"logs/etc_15m_trader_{datetime.now().strftime('%Y%m%d')}.log"),
        logging.StreamHandler(),
    ],
)
log = logging.getLogger("etc_15m_trader")

# ── State ─────────────────────────────────────────────────────────────────────
STATE_FILE = "etc_15m_state.pkl"
