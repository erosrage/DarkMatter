"""
SOL 15m Grid Trader -- Production
...
Strategy: reactive counter-orders, no periodic reset.
  ...
  - On buy fill: immediately place a counter-sell at +9.0% (step_pct x cm)
  - On sell fill: immediately place a counter-buy at -9.0% if cash available
  - No periodic cancel-and-replace; orders persist until filled

Optimised params (backtest: train=+33.63%, val=+12.79% over ~60 days on $100k):
  sweep=0.15  steps=20  order_pct=0.50  counter_mult=12
...
STEPS           = 20        # buy levels in the initial grid
...
# step_pct = SWEEP / STEPS = 0.15 / 20 = 0.75% per step
# counter distance = step_pct * COUNTER_MULT = 9.0% above/below fill price
...
logging.basicConfig(
    ...
    datefmt="%Y-%m-%d %H:%M:%S",
    handlers=[
        logging.FileHandler(f"logs/sol_15m_trader_{datetime.now().strftime('%Y%m%d')}.log"),
        logging.StreamHandler(),
    ],
)
log = logging.getLogger("sol_15m_trader")

# ── State ─────────────────────────────────────────────────────────────────────
STATE_FILE = "sol_15m_state.pkl"
...
# ── Grid ──────────────────────────────────────────────────────────────────────
STEP_PCT     = SWEEP / STEPS   # 0.0075
QTY_DECIMALS = 2

def price_dp(price):
    """Decimal places so each grid step spans at least 1 tick."""
...
async def place_buy(level, qty):
    log.info(f"  BUY  {qty:.{QTY_DECIMALS}f} {SYMBOL} @ ${level:.4f}")
    try:
...
async def place_sell(level, qty):
    log.info(f"  SELL {qty:.{QTY_DECIMALS}f} {SYMBOL} @ ${level:.4f}")
    try:
