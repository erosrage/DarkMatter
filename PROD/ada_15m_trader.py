"""
ADA 15m Grid Trader -- Production
Optimised params from full brute-force param search (research/param_search_full.py).
...
Strategy: reactive counter-orders, no periodic reset.
  ...
  - On buy fill: immediately place a counter-sell at +6.25% (step_pct x cm)
  - On sell fill: immediately place a counter-buy at -6.25% if cash available
  - No periodic cancel-and-replace; orders persist until filled

Optimised params (backtest: train=+30.91%, val=+11.00% over ~60 days on $100k):
  sweep=0.25  steps=48  order_pct=0.50  counter_mult=12

Note: with order_pct=0.50 and 48 steps, only the 2 closest buy levels are placed
at startup (buying power is consumed after 2 orders at 50% each). The remaining
steps serve as reference levels for counter-order placement only.
...
# ── Configuration ─────────────────────────────────────────────────────────────
SYMBOL          = "ADA"
SWEEP           = 0.25      # grid width each side (25%)
STEPS           = 48        # buy levels in the initial grid
ORDER_SIZE_PCT  = 0.50      # fraction of available buying power per order level
COUNTER_MULT    = 12.0      # counter-order gap multiplier (step_pct x COUNTER_MULT)
INTERVAL        = 900       # seconds between cycles (15 minutes — matches backtest cadence)
CIRCUIT_BREAKER = 0.20      # halt if portfolio drops >20% from starting value

# step_pct = SWEEP / STEPS = 0.25 / 48 ≈ 0.521% per step
# counter distance = step_pct * COUNTER_MULT ≈ 6.25% above/below fill price
...
logging.basicConfig(
    ...
    datefmt="%Y-%m-%d %H:%M:%S",
    handlers=[
        logging.FileHandler(f"logs/ada_15m_trader_{datetime.now().strftime('%Y%m%d')}.log"),
        logging.StreamHandler(),
    ],
)
log = logging.getLogger("ada_15m_trader")

# ── State ─────────────────────────────────────────────────────────────────────
STATE_FILE = "ada_15m_state.pkl"
...
async def _relogin():
    """Re-authenticate when the session token has expired (401)."""
...
async def place_buy(level, qty):
    log.info(f"  BUY  {qty:.{QTY_DECIMALS}f} {SYMBOL} @ ${level:.6f}")
    try:
        ...
        if not (resp and isinstance(resp, dict) and resp.get("id")):
            log.warning(f"  Buy order rejected: {resp}")
        return resp
    except Exception as e:
...
async def place_sell(level, qty):
    log.info(f"  SELL {qty:.{QTY_DECIMALS}f} {SYMBOL} @ ${level:.6f}")
    try:
...
async def run_cycle(state):
    ...
    log.info(f"{SYMBOL} price: ${price:.6f}")
    ...
    if not tracked:
        ...
        for o in disappeared:
            ...
            if order_state in ("filled", "partially_filled"):
                ...
                log.info(f"  Order {o['id'][:8]}... was {order_state} -- skipping counter")
        ...
        if filled:
            ...
            for order in filled:
                ...
                if order["side"] == "buy":
                    ...
                    log.info(f"  fill BUY @ {order['price']:.6f} -> counter SELL @ {counter_price:.6f}")
                else:
                    counter_price = round_price(order["price"] * (1 - sp * COUNTER_MULT), order["price"])
                    order_usd     = buying_power * ORDER_SIZE_PCT
                    if buying_power >= order_usd and order_usd > 0:
                        ...
                        log.info(f"  fill SELL @ {order['price']:.6f} -> counter BUY @ {counter_price:.6f}")
                    else:
                        log.info(f"  fill SELL @ {order['price']:.6f} -> counter BUY skipped (insufficient cash)")
...
async def main():
    ...
    log.info(
        f"{SYMBOL} 15m Grid Trader started | "
        f"sweep={SWEEP} steps={STEPS} order_pct={ORDER_SIZE_PCT} "
        f"cm={COUNTER_MULT} interval={INTERVAL}s circuit_breaker={CIRCUIT_BREAKER:.0%}"
    )
