"""
SOL Grid Trader — Production
Optimized settings derived from 5-year backtest (sweep=0.30, steps=16) → +165.2%.
...
SYMBOL         = "SOL"
SWEEP          = 0.30      # base grid range (overridden by ATR when enough data exists)
STEPS          = 16        # buy + sell levels per side
ORDER_SIZE     = 0.01      # SOL per order (base; pyramid scales deeper levels)
...
log = logging.getLogger("sol_trader")

# ── State persistence ─────────────────────────────────────────────────────────

STATE_FILE = "sol_state.pkl"
...
async def get_portfolio_value(price):
    """5. Fetch current buying power + SOL position value."""
    try:
        ...
        for pos in positions:
            if pos.get("currency", {}).get("code") == "SOL":
...
async def cancel_open_sol_orders():
    """4. Cancel all open SOL crypto limit orders before placing new ones."""
    try:
        orders = await asyncio.to_thread(rh.orders.get_all_open_crypto_orders)
        sol_orders = [
            o for o in orders
            if "SOL" in str(o.get("currency_pair_id", "")).upper()
        ]
        if sol_orders:
            log.info(f"Cancelling {len(sol_orders)} stale SOL order(s)...")
...
async def main():
    ...
    log.info(
        f"SOL Grid Trader started  sweep={SWEEP}(ATR-adjusted)  steps={STEPS}  "
        f"order={ORDER_SIZE} SOL  pyramid={PYRAMID}  "
