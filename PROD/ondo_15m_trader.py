"""
ONDO 15m Grid Trader -- Production
...
Optimised params (backtest: train=+36.37%, val=+11.97% over ~60 days on $100k):
  sweep=0.05  steps=8  order_pct=0.50  counter_mult=10
...
# ── Configuration ─────────────────────────────────────────────────────────────
SYMBOL          = "ONDO"
SWEEP           = 0.05      # grid width each side (5%)
STEPS           = 8         # buy levels in the initial grid
ORDER_SIZE_PCT  = 0.50      # fraction of available buying power per order level
COUNTER_MULT    = 10.0      # counter-order gap multiplier (step_pct x COUNTER_MULT)
...
# step_pct = SWEEP / STEPS = 0.05 / 8 = 0.625% per step
...
logging.basicConfig(
    ...
    datefmt="%Y-%m-%d %H:%M:%S",
    handlers=[
        logging.FileHandler(f"logs/ondo_15m_trader_{datetime.now().strftime('%Y%m%d')}.log"),
        logging.StreamHandler(),
    ],
)
log = logging.getLogger("ondo_15m_trader")

# ── State ─────────────────────────────────────────────────────────────────────
STATE_FILE = "ondo_15m_state.pkl"
