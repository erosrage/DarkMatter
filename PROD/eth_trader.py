...
Optimized settings derived from 2-year backtest (sweep=0.40, steps=2).
...
SYMBOL         = "ETH"
SWEEP          = 0.40      # base grid range (overridden by ATR when enough data exists)
STEPS          = 2         # buy + sell levels per side
ORDER_SIZE     = 0.01      # ETH per order (base; pyramid doubles the deeper level)
...
# 3. Pyramid sizing
PYRAMID        = True      # deeper level gets 2× ORDER_SIZE

# 4. Stale order cancellation — done at the start of every cycle (see cancel_open_eth_orders)

# 5. Circuit breaker
CIRCUIT_BREAKER      = 0.20   # halt if portfolio drops 20% below starting value
DROP_THRESHOLD       = 0.08   # risk guard: also suppress buys on 8% intra-session drop
...
logging.basicConfig(
    ...
    datefmt="%Y-%m-%d %H:%M:%S",
    handlers=[
        logging.FileHandler(f"logs/eth_trader_{datetime.now().strftime('%Y%m%d')}.log"),
        logging.StreamHandler(),
    ],
)
log = logging.getLogger("eth_trader")

# ── State persistence ─────────────────────────────────────────────────────────

STATE_FILE = "eth_state.pkl"
...
def trend_is_up(price):
    ...
    if len(price_history) >= MA_PERIOD:
        ...
        return price > ma
    return True  # not enough history yet — allow buys
...
async def get_portfolio_value(price):
    """5. Fetch current buying power + ETH position value."""
    try:
        ...
        for pos in positions:
            if pos.get("currency", {}).get("code") == "ETH":
                eth_qty = float(pos.get("quantity", 0))
        return buying_power + eth_qty * price
...
async def cancel_open_eth_orders():
    """4. Cancel all open ETH crypto limit orders before placing new ones."""
    try:
        orders = await asyncio.to_thread(rh.orders.get_all_open_crypto_orders)
        eth_orders = [
            o for o in orders
            if o.get("currency_pair_id", "").upper().startswith("ETH")
            or "ETH" in str(o.get("currency_pair_id", "")).upper()
        ]
        if eth_orders:
            log.info(f"Cancelling {len(eth_orders)} stale ETH order(s)...")
...
async def place_buy(price, size):
    log.info(f"  BUY  {size} ETH @ ${price:,.2f}")
    return await asyncio.to_thread(
        rh.orders.order_buy_limit, SYMBOL, size, price
    )

async def place_sell(price, size):
    log.info(f"  SELL {size} ETH @ ${price:,.2f}")
    return await asyncio.to_thread(
        rh.orders.order_sell_limit, SYMBOL, size, price
    )
...
async def run_cycle(state):
    ...
    await cancel_open_eth_orders()

    # ── 1. Trend filter ───────────────────────────────────────────────────────
    trend_up = trend_is_up(price)

    # ── Risk guard (intra-session crash) ──────────────────────────────────────
    ...
    if trend_up and not paused:
        for idx, b in enumerate(buys):
            # ── 3. Pyramid: deeper level gets 2× size ─────────────────────
...
async def main():
    ...
    log.info(
        f"ETH Grid Trader started  sweep={SWEEP}(ATR-adjusted)  steps={STEPS}  "
        f"order={ORDER_SIZE} ETH  pyramid={PYRAMID}  "
