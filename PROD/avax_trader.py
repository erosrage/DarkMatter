...
Strategy: reactive counter-orders, no periodic reset.
  ...
  - On buy fill: immediately place a counter-sell at +7.2% (step_pct x cm)
  - On sell fill: immediately place a counter-buy at -7.2% if cash available
  - No periodic cancel-and-replace; orders persist until filled

Optimised params (backtest result: +26.34% over 34 days on $100k):
  sweep=0.12  steps=20  order_pct=0.20  counter_mult=12.0
...
# ── Configuration ─────────────────────────────────────────────────────────────
SYMBOL         = "AVAX"
SWEEP          = 0.12       # grid width each side (12%)
STEPS          = 20         # buy levels (and sell levels) in the initial grid
ORDER_SIZE_PCT = 0.20       # fraction of available buying power per order level
...
# step_pct = SWEEP / STEPS = 0.12 / 20 = 0.60% per step
# counter distance = step_pct * COUNTER_MULT = 7.2% above/below fill price
...
logging.basicConfig(
    ...
    datefmt="%Y-%m-%d %H:%M:%S",
    handlers=[
        logging.FileHandler(f"logs/avax_trader_{datetime.now().strftime('%Y%m%d')}.log"),
        logging.StreamHandler(),
    ],
)
log = logging.getLogger("avax_trader")

# ── State ─────────────────────────────────────────────────────────────────────
STATE_FILE = "avax_state.pkl"
