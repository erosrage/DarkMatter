...
Strategy: reactive counter-orders, no periodic reset.
  ...
  - On buy fill: immediately place a counter-sell at +4.69% (step_pct x cm)
  - On sell fill: immediately place a counter-buy at -4.69% if cash available
  - No periodic cancel-and-replace; orders persist until filled

Optimised params (backtest result: +35.56% over 34 days on $100k):
  sweep=0.15  steps=32  order_pct=0.50  counter_mult=10.0
...
STEPS          = 32         # buy levels (and sell levels) in the initial grid
...
# step_pct = SWEEP / STEPS = 0.15 / 32 = 0.469% per step
# counter distance = step_pct * COUNTER_MULT = 4.69% above/below fill price
...
logging.basicConfig(
    ...
    datefmt="%Y-%m-%d %H:%M:%S",
    handlers=[
        logging.FileHandler(f"logs/ondo_trader_{datetime.now().strftime('%Y%m%d')}.log"),
        logging.StreamHandler(),
    ],
)
log = logging.getLogger("ondo_trader")

# ── State ─────────────────────────────────────────────────────────────────────
STATE_FILE = "ondo_state.pkl"
...
# ── Grid ──────────────────────────────────────────────────────────────────────
STEP_PCT = SWEEP / STEPS   # 0.00469

QTY_DECIMALS = 2   # Robinhood quantity precision

def price_dp(price):
    """Decimal places so each grid step spans at least 1 tick (no collapsed levels)."""
...
async def get_portfolio_value(price, open_orders):
    try:
        ...
        # Crypto limit buy orders reserve funds not visible in buying_power;
        # add them back from our tracked order list.
...
async def place_buy(level, qty):
    log.info(f"  BUY  {qty:.6f} {SYMBOL} @ ${level:.6f}")
    try:
...
async def place_sell(level, qty):
    log.info(f"  SELL {qty:.6f} {SYMBOL} @ ${level:.6f}")
    try:
...
async def run_cycle(state):
    ...
    if not tracked:
        ...
        # Verify each disappeared order is filled, not cancelled/rejected
        filled = []
